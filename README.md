if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Domes Softwares is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Domes Softwares is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Domes Softwares FE v" .. currentVersion
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://discord.gg/hhbbBnWzug")
		...
		inviteButton.Text = "Copy Discord Invite Link (https://discord.gg/hhbbBnWzug)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Domes Softwares Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Domes Softwares support server.'}
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://discord.com/invite/hhbbBnWzug')
		notify('Discord Invite', 'Copied to clipboard!\ndiscord.gg/hhbbBnWzug')
	else
		notify('Discord Invite', 'discord.gg/hhbbBnWzug')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Domes Softwares may have issues or features that are not functioning correctly.") end
end)
